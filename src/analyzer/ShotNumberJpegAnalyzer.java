...
public class ShotNumberJpegAnalyzer implements Analyzer
{
	private String filePath;
	private double keysFound = 0.0; //keys actually found in file		
	private ArrayList<KeyValuePair> keyAndTypeList ;
	
	public ShotNumberJpegAnalyzer(String path)
    {
    	this.filePath = path;
    }
	
	/**
	 * Extracts keys and values from a Jpeg file
	 ...
	 * @return: An ArrayList of KeyValuePairs extracted from the Jpeg file
	...
	private ArrayList<KeyValuePair> extractFile() 
	{
		...
		try
		{
		 ...
		 {
			 System.err.println("ShotNumberJpegAnalyzer: File does not exist or is not a file ");
			 return null;
		 }
		}//end try
		catch(Exception e )
		{
			System.err.println("ShotNumberJpegAnalyzer: Error reading image file "+ e.getMessage());
			return null;
		}
		return pairList;
	}
